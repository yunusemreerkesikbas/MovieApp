...
export class MovieService {
  ...
  getTopMovies(): Observable<MovieRoot> {
    let url = `${environment.ROOT_URL}movie/popular?api_key=${environment.MOVIEDB_API_KEY}`;
    return this.httpClient.get<MovieRoot>(url);
  }
  getMoviesByCategory(category: string): Observable<MovieRoot> {
    let url = `${environment.ROOT_URL}movie/${category}?api_key=${environment.MOVIEDB_API_KEY}`;
    return this.httpClient.get<MovieRoot>(url);
  }
